import { LayoutGrid, Mars, Venus, Users, User } from "lucide-react"; // Import Icon Baru

export default function MobileNavbar({ currentPage, onNavigate }) {
  const navItems = [
    { id: "home", label: "Dashboard", icon: LayoutGrid },
    { id: "pria", label: "Pria", icon: Mars }, // Icon Pria (Mars)
    { id: "wanita", label: "Wanita", icon: Venus }, // Icon Wanita (Venus)
    { id: "unisex", label: "Unisex", icon: Users }, // Menu Baru
    { id: "profile", label: "Profil", icon: User },
  ];

  return (
    <nav className="fixed bottom-0 left-1/2 -translate-x-1/2 w-fit min-w-[350px] max-w-[95%] bg-white/95 backdrop-blur-xl border-t border-x border-gray-200/80 rounded-t-3xl pb-safe z-50 shadow-[0_-8px_30px_-10px_rgba(0,0,0,0.1)]">
      <div className="flex items-end justify-around gap-1 px-4 py-3 pb-4">
        {navItems.map((item) => {
